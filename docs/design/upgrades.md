...
**Measured (harness, first pass): 128 damage per full transit against a Shielded Scout.** Two shells at 64 — the damage table's worst cell (explosive vs Shielded, 40%) doing exactly what it was built to do. The measurement triggered the right audit, though: *is this tier ladder silently compensating for an under-tuned base tower?* Checked against the ladder itself: **no.** Deep Barrel is the smallest tier-2 jump in the game (+55% DPS, against the Arc's +100% and the Lance's +81%); Glacier deliberately holds damage flat — it buys seconds, not hit points, and says so above; Shatterhead is a role pivot, not a patch. The tiers scale a time-seller; they do not paper over one.

What the measurement actually exposed is a reporting gap: the mortar's product is the slow, and a damage-per-tower column cannot see it. The directive and the floor now live in [difficulty.md](./difficulty.md) §5 — report *added exposure-seconds* per tower; mortar-inclusive S2 policies must not underperform mortar-less same-spend ones. If that floor fails, the base tower is the thing to touch, and the first knob is the **90-tick interval, not the damage**: at drone speed a transit of the mortar's circle fits two shells, and whether the second lands is entry-timing luck — 75 ticks makes it three reliably, sits inside the declared band, and respects the identity. A mortar tuned to kill things is a different tower and a worse one.
...
*Snapshot status (12 Aug 2026, after the first balance pass):* the shipped tiers moved several §4–6 absolutes (Arc base costs 110; Lance base 400 dmg at 160; Prime Focus 1,180 @ 90t; the Mortar family runs 68/62/62/58-tick intervals — the shots-per-transit fix — with splash 3.75/4.5/5.6/4.25) and **every family invariant checked and held**: Hailstorm's 436 single-target DPS still sits under Overclock's 450, Railshot vs Shielded is still the worst tier-3, Glacier still owns the deepest slow, Shatterhead single-target still trails every Lance tier (narrowest margin in the set, ~3%). The doctrine is the spec; the tables above are its pre-pass snapshot and are left as drafted.

Two post-harness amendments. First: starting credits moved outside their declared band by measured necessity (difficulty.md §6–7). If that same economy rescale reaches this file's cost ladder, the invariants are the ladder's *order* and §3's marginal-efficiency doctrine — not the absolute prices. Crossing ±25% on costs during that one rescale inherits A1's standing exception rather than needing a fresh round-trip; everything else still comes back here. Second: difficulty.md §5 now carries a failing anchor whose likely fix moves the Arc Cannon base to ~110 damage. If it lands, **Overclock rescales with it under its own doctrine** — tier 2 equals two of the current base in one footprint (≈ 550 DPS at a 110 base), not the stale absolute in §4's table. The doctrine is the spec; the table is its snapshot.
